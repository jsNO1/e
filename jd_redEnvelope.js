...
function getCoupons(shareId = '') {
  return new Promise(resolve => {
    let options = {
      url: `https://api.m.jd.com/api?functionId=getCoupons&appid=u&_=${Date.now()}&loginType=2&body=${encodeURIComponent(JSON.stringify({"platform":4,"unionActId":"31134","actId":$.actId,"d":"3wVdViu","unionShareId":shareId,"type":1,"eid":$.eid}))}&client=apple&clientVersion=8.3.6&h5st=undefined`,
      headers: {
        "Host": "api.m.jd.com",
...
function shareUnionCoupon() {
  return new Promise(resolve => {
    let options = {
      url: `https://api.m.jd.com/api?functionId=shareUnionCoupon&appid=u&_=${Date.now()}&loginType=2&body=${encodeURIComponent(JSON.stringify({"unionActId":"31134","actId":$.actId,"platform":4,"unionShareId":$.shareCode,"d":"3wVdViu","supportPic":2,"supportLuckyCode":0,"eid":$.eid}))}&client=apple&clientVersion=8.3.6`,
      headers: {
        "Host": "api.m.jd.com",
        "Origin": "https://prodev.m.jd.com",
        "User-Agent": UA,
        ...
        if (err) {
          ...
          if (typeof res == 'object') {
            if (res.code == 0 && res.data && res.data.shareUrl) {
              $.shareCode = res.data.shareUrl.match(/3wVdViu\?s=([^&]+)/) && res.data.shareUrl.match(/3wVdViu\?s=([^&]+)/)[1] || ''
...
function getUrl() {
  return new Promise(resolve => {
    const options = {
      url: `https://u.jd.com/3wVdViu?s=${$.shareCode}`,
      followRedirect:false,
      headers: {
        "Host": "u.jd.com",
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",

...
const BASE_URL1 = 'https://wq.jd.com/cubeactive/steprewardv3'
!(async () => {
  if (!cookiesArr[0]) {
...
function enrollFriend(strPin) {
  return new Promise(resolve => {
    ...
    const options = taskurl1('EnrollFriend', body, 'activeId,channel,joinDate,phoneid,publishFlag,strPin,timestamp');
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          console.log(`\n${$.name}:  API查询请求失败 ‼️‼️`)
...
function taskurl1(function_path, body = '', stk) {
  let url = `${BASE_URL1}/${function_path}?activeId=${$.activeId}&publishFlag=1&channel=7&${body}&sceneval=2&g_login_type=1&timestamp=${Date.now()}&_=${Date.now() + 2}&_ste=1`
  const deviceId = UA.split(';') && UA.split(';')[4] || ''
  url += `&phoneid=${deviceId}`
  url += `&stepreward_jstoken=${
    Math.random().toString(36).slice(2, 10) +

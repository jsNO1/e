...
let allMessage = '';
let joyRewardName20 = 20;//是否兑换京豆，默认0不兑换京豆，其中20为兑换20京豆,500为兑换500京豆，0为不兑换京豆.数量有限先到先得
  ...
  try {
    ...
    for (let j = 0; j <= 10; j++) {
      ...
      if ($.getExchangeRewardsRes && $.getExchangeRewardsRes.success) {
        ...
        let rewardNum = 0;
        if ($.isNode() && process.env.JD_JOY_REWARD_NAME2020) {
          rewardNum = process.env.JD_JOY_REWARD_NAME2020 * 1;
        } else if ($.getdata('joyRewardName20')) {
          if ($.getdata('joyRewardName20') * 1 === 1) {
            //兼容之前的BoxJs设置
            rewardNum = 20;
          } else {
            rewardNum = $.getdata('joyRewardName20') * 1;
          }
        } else {
          rewardNum = joyRewardName20;
        }
        ...
        if (rewardNum && (rewardNum === 1 || rewardNum === 20 || rewardNum === 50 || rewardNum === 100 || rewardNum === 500 || rewardNum === 1000)) {
          ...
          console.log(`\n您设置了不兑换京豆,如需兑换京豆，请去BoxJs处设置或修改joyRewardName20代码或设置环境变量 JD_JOY_REWARD_NAME20`)
        }
      } else {

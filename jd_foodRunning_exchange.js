...
async function main() {
  ...
  // await $.wait(3000);
  console.log(`打印兑换列表`);
  await takePostRequest('AwardList');
  console.log(`尝试兑换`);
  await takePostRequest('exchangeGoods');
...
async function takePostRequest(type){
  ...
  switch (type) {
    ...
    case 'AwardList':
      url = `https://jinggengjcq-isv.isvjcloud.com/dm/front/foodRunning/${type}?open_id=&mix_nick=&bizExtString=&user_id=10299171`;
      body =  {"buyerNick":$.thisNick,"method":"/foodRunning/"+type,"pushWay":1,"actId":"jd_food_running","userId":"10299171"};
      break;
      case 'exchangeGoods':
        url = `https://jinggengjcq-isv.isvjcloud.com/dm/front/foodRunning/${type}?open_id=&mix_nick=&bizExtString=&user_id=10299171`;
        body =  {"buyerNick":$.thisNick,"method":"/foodRunning/"+type,"pushWay":1,"awardId":"jdAward3","actId":"jd_food_running","userId":"10299171"};
        break;
    default:
      console.log(`错误${type}`);
  }
...
function dealReturn(type, data) {
  ...
  switch (type) {
    ...
    case 'exchangeGoods':
      if(data.success && data.errorCode === '200' && data.data && data.data.status && data.data.status === 200){
        $.myInviteList = data.data.data;
        console.log(`兑换结果：${data.data.data.msg}`)
      }else{
        console.log(JSON.stringify(data));
      }
      break;

...
[task_local]
#愤怒的锦鲤
59 23 * * *  jd_redPacket.js, tag=愤怒的锦鲤, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_redPacket.png, enabled=true
===================Loon==============
[Script]
cron "59 23 * * * " script-path=jd_redPacket.js, tag=愤怒的锦鲤
===============Surge===============
[Script]
愤怒的锦鲤 = type=cron,cronexp="59 23 * * *",wake-system=1,timeout=3600,script-path=jd_redPacket.js
====================================小火箭=============================
愤怒的锦鲤 = type=cron,script-path=jd_redPacket.js, cronexpr="59 23 * * *", timeout=3600, enable=true
愤怒的锦鲤
更新时间：2021-7-11
备注：高速并发请求，专治偷助力。在kois环境变量中填入需要助力的pt_pin，有多个请用@符号连接
TG学习交流群：https://t.me/cdles
https://raw.githubusercontent.com/cdle/jd_study/main/jd_angryKoi.js
*/
const $ = new Env("愤怒的锦鲤")
...
!(async () => {
    ...
    console.log(`脚本开始助力时间 ${(new Date()).Format("yyyy-MM-dd hh:mm:ss | S")}`);
    for (let i = 0; i < len; i++) {
        cookie = cookiesArr[i]
        if(!kois){
            if(i != 0) {
